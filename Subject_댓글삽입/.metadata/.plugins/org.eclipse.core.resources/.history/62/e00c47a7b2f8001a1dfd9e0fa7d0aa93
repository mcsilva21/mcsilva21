			...
			<table border="1">
				<thead>
					<tr>
						...
						<th><a href="?${pager.query}&header=2&order=${pager.header == 2 ? (pager.order + 1) % 2 : 0}">학생이름</a></th>
						<th><a href="?${pager.query}&header=3&order=${pager.header == 3 ? (pager.order + 1) % 2 : 0}">학점</a></th>
						<th><a href="?${pager.query}&header=4&order=${pager.header == 4 ? (pager.order + 1) % 2 : 0}">신청일자</a></th>
						<th>관리</th>
					</tr>
				</thead>
				<tbody>
					<c:choose>
						<c:when test="${list.size() > 0}">
							<c:forEach var="item" items="${list}">			
								<tr>
									<td><a href="${item.studyid}/view">${item.studyid}</a></td>
									<td>${item.stuname}</td>
