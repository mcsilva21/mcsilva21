			...
			<form method="get" action="">
				<span>
					<select name="search">
						...
						<option value="3" ${pager.search == 4 ? 'selected' : ''}>학점</option>
						<option value="4" ${pager.search == 5 ? 'selected' : ''}>신청날짜</option>
			...
			<table border="1">
				<thead>
					<tr>
						...
						<th><a href="?${pager.query}&header=2&order=${pager.header == 2 ? (pager.order + 1) % 2 : 0}">학생번호</a></th>
						<th><a href="?${pager.query}&header=3&order=${pager.header == 3 ? (pager.order + 1) % 2 : 0}">학점</a></th>
						<th><a href="?${pager.query}&header=4&order=${pager.header == 7 ? (pager.order + 1) % 2 : 0}">신청일자</a></th>
						<th>관리</th>
					</tr>
				</thead>
				<tbody>
