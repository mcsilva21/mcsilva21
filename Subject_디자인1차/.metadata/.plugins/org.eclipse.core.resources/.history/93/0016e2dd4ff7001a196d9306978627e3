			...
			<form method="get" action="">
				<span>
					<select name="search">
						...
						<option value="2" ${pager.search == 2 ? 'selected' : ''}>교객번호</option>
						<option value="3" ${pager.search == 3 ? 'selected' : ''}>도서번호</option>
						<option value="4" ${pager.search == 4 ? 'selected' : ''}>판매가격</option>
						<option value="5" ${pager.search == 5 ? 'selected' : ''}>주문날짜</option>
			...
			<table border="1">
				<thead>
					<tr>
						...
						<th><a href="?${pager.query}&header=2&order=${pager.header == 2 ? (pager.order + 1) % 2 : 0}">고객번호</a></th>
						<th><a href="?${pager.query}&header=3&order=${pager.header == 3 ? (pager.order + 1) % 2 : 0}">도서번호/a></th>
						<th><a href="?${pager.query}&header=4&order=${pager.header == 4 ? (pager.order + 1) % 2 : 0}">판메가격</a></th>
						<th><a href="?${pager.query}&header=5&order=${pager.header == 5 ? (pager.order + 1) % 2 : 0}">판메일자</a></th>
						<th>관리</th>
					</tr>
				</thead>
				<tbody>
